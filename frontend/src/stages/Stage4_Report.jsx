  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: "24px" }}>
        <style>{`
          @keyframes veridex-spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
          @keyframes veridex-pulse { 0%,100% { opacity:1; } 50% { opacity:0.4; } }
        `}</style>
        <div style={{ width: "64px", height: "64px", border: "4px solid rgba(240,165,0,0.2)", borderTopColor: "#f0a500", borderRadius: "50%", animation: "veridex-spin 1s linear infinite" }} />
        <div style={{ textAlign: "center" }}>
          <h2 style={{ fontSize: "22px", fontWeight: "700", color: "white", animation: "veridex-pulse 2s ease-in-out infinite", margin: 0 }}>
            Synthesizing REAL-TIME Intelligence...
          </h2>
          <p style={{ color: "rgba(255,255,255,0.5)", marginTop: "8px", fontSize: "14px" }}>
            Fetching latest RBI flags and financial news
          </p>
        </div>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "center" }}>
          {["Extracting financials", "Running web research", "Calculating score", "Building report"].map((s, i) => (
