    ...
    try:
        ...
        if isinstance(extracted_data, list):
            for doc in extracted_data:
                ...
                if fields:
                    extracted_docs[doc_type] = fields
        ...
        scoring_result = {
            ...
            "swot": score_data.get("swot") or research_data.get("swot") or {
                "strengths": [s for s in score_data.get("green_flags", [])[:3]] or ["Established market presence", "Regulatory compliance"],
                "weaknesses": [s for s in score_data.get("red_flags", [])[:3]] or ["Requires further due diligence"],
                "opportunities": ["Growing credit demand in India", "Digital lending expansion"],
                "threats": ["Regulatory tightening on NBFCs", "Rising cost of funds"]
            }
        }

        # Get research findings - try multiple keys
        research_findings = research_data.get("findings", [])
        if not research_findings:
            # Convert latest_news strings to finding dicts
            latest_news = research_data.get("latest_news", [])
            if latest_news:
                research_findings = [{"title": n, "snippet": n, "url": ""} for n in latest_news if n and n != "null"]
        # Filter out nulls
        research_findings = [f for f in research_findings if f and f.get("title") and f.get("title") != "null"]
        ...
        def get_cs(key, max_v):
            d = five_cs.get(key, {})
            score_v = d.get("score", 0) if isinstance(d, dict) else (d or 0)
            notes_v = d.get("notes", "Based on extracted financial data") if isinstance(d, dict) else ""
            return [f"{score_v}/{max_v}", notes_v]
        cs_rows = [
            ["Character (Promoter Background)"] + get_cs("character", 20),
            ["Capacity (Revenue & Profit)"] + get_cs("capacity", 20),
            ["Capital (Net Worth & Leverage)"] + get_cs("capital", 20),
            ["Collateral (Asset Coverage)"] + get_cs("collateral", 20),
            ["Conditions (Sector Outlook)"] + get_cs("conditions", 15),
        ]
        ...
        def fmt_cr(v):
            sv = safe_val(v)
            return f"₹{sv} Cr" if sv else "N/A"
        def fmt_pct(v):
            sv = safe_val(v)
            return f"{sv}%" if sv else "N/A"

        fin_rows = [
            ["Revenue", fmt_cr(annual.get("revenue")), "Total income from operations"],
            ["Net Profit (PAT)", fmt_cr(annual.get("pat")), "Profit After Tax"],
            ["EBITDA", fmt_cr(annual.get("ebitda")), "Earnings before interest, tax, depreciation"],
            ["Total Debt", fmt_cr(annual.get("total_debt") or borrowing.get("total_debt")), "Total borrowings"],
            ["Net Worth", fmt_cr(annual.get("net_worth")), "Shareholders equity"],
            ["Total Assets", fmt_cr(annual.get("total_assets") or alm.get("total_assets")), "Total assets"],
            ["Gross NPA %", fmt_pct(annual.get("gnpa_percent") or portfolio.get("gnpa_percent")), "Below 2% is healthy"],
            ["CAR %", fmt_pct(annual.get("car_percent")), "Above 15% is adequate (RBI)"],
            ["Interest Coverage", safe_val(annual.get("interest_coverage")) or "N/A", "EBIT / Interest Expense"],
            ["Cash from Operations", fmt_cr(annual.get("cash_from_operations")), "Operating cash flow"],
            ["Credit Rating", safe_val(borrowing.get("credit_rating_long_term")) or "N/A", "Long-term rating"],
            ["Rating Outlook", safe_val(borrowing.get("rating_outlook")) or "N/A", "Rating trajectory"],
            ["Promoter Holding", fmt_pct(shareholding.get("promoter_holding")), "Promoter stake"],
            ["Pledged Shares", fmt_pct(shareholding.get("pledged_shares")), "0% is ideal"],
        ]
        ...
        if research_findings:
            ...
            if news:
                ...
                doc.add_paragraph("No adverse web findings detected. All secondary research sources returned clean results for this entity.")
